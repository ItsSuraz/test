...
const app = express();
const port = config.Verification.https ? 443 : config.Verification.Port;
...
// GET /verify/id
app.get('/verification/:verifyId?', (req, res) => {
    ...
    res.render(path.join(__dirname, '/html/verify.html'), { publicKey: config.Verification['public-key'] });
});

// POST /verify/id
app.post('/verification/:verifyId?', async (req, res) => {
    if (!req.body || !req.body['g-recaptcha-response']) return res.sendFile(path.join(__dirname, '/html/invalidLink.html'));

    const response = await axios({
        method: 'post',
        url: `https://www.google.com/recaptcha/api/siteverify?secret=${config.Verification['secret-key']}&response=${req.body['g-recaptcha-response']}`,
        headers: {
